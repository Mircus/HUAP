...
This is the canonical implementation — installed as part of hu-plugins-hindsight
and loaded by core via the plugin boundary.
"""
...
from hu_core.memory.providers.base import (
    MemoryProvider,
    MemoryEntry,
    MemoryQuery,
    MemoryType,
...
from hu_core.policies.memory_ingest import redact_secrets
...
class HindsightProvider(MemoryProvider):
    ...
    - Automatic secret redaction on all writes
    """
    ...
    async def set(self, entry: MemoryEntry) -> MemoryEntry:
        """Store a memory entry (insert or replace). Secrets are always redacted."""
        self._ensure_connected()
        # Redact secrets before persistence — unavoidable choke point
